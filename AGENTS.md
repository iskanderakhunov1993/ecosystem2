...
Mira — приватный дневник женского здоровья. Продукт превращает короткие
...
**`apps/mira`** — Vite + React 18 + Zustand + IndexedDB. **База продукта.**
Шесть жизненных режимов, онбординг, Life-Stage Gate, ступени уверенности,
честные пустые состояния. Порт разработки 5180.

**`apps/web`** — Next.js 15 + Zustand + localStorage + Supabase. Более старое
и более полное приложение: авторизация, анализы с референсами, контент,
синхронизация, партнёрский доступ, исламский режим. Порт 4173.

Направление развития: `apps/mira` — основа, из `apps/web` в неё переносится
зрелая функциональность. Не наоборот.
