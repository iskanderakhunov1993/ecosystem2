...
> них не построены.
>
> Это осознанно. Чат-ассистент по здоровью — дорого, медицински рискованно и
> не решает главную задачу продукта: доказательство для врача строится на
> фактах пользователя, а не на сгенерированном тексте. Правило `AGENTS.md`:
> реальные AI-вызовы не добавляются без отдельной постановки задачи.
>
> Документ описывает целевую архитектуру на случай, когда AI понадобится, и
> фиксирует границы безопасности заранее.
